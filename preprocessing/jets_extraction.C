...
void jets_extraction(const char *filename)
{
	...
	*/
	...
	// create first mask
	auto d_def = d.Define("JetMask", "Jet_genJetIdx >=0  && Jet_genJetIdx < nGenJet").Define("MatchedGenJets", "Jet_genJetIdx[JetMask]").Define("MuonMask", "GenPart_pdgId == 13 | GenPart_pdgId == -13");
	...
	d_matched.Snapshot("MJets", "data/jets/" + hash + ".root", col_to_save);
}
